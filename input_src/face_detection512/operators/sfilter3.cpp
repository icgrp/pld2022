...
#pragma HLS unroll
	  ...
	  //}
    	...
    	out_tmp3(12, 0) = I_3[4][30](12, 0);
    	out_tmp3(52, 32) = SI_3[4][30](20, 0);
...
#pragma HLS unroll
		if( j != 2*WINDOW_SIZE-16 ){
		  ...
		  I_3[0][j] = I_3[0][j+1] + in_tmp2(12, 0);
		  SI_3[0][j] = SI_3[0][j+1] + in_tmp2(52, 32);
		}
        for( i = 1; i < 5; i++ ){
        #pragma HLS unroll
	 ...
	 /* Sliding Window should not go beyond the boundary */
	     ...
	     #pragma HLS array_partition variable=sum complete dim=0
            ...
            {
#pragma HLS unroll
           		 out_tmp(i*12+11, i*12) = (ap_int<12>) (sum[i]/1048576);
            }
