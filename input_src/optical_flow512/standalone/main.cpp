...
int main(int argc, char ** argv) 
{
  ...
  // sdsoc version host code
    ...
    //  }
   // }
    ap_uint<512>  tmpframes;
    //static hls::stream<stdio_t> frames("test1");
    static hls::stream< ap_uint<512> > frames("test1");
    static hls::stream< ap_uint<512> > flo_out("test2");
     // pack the values
    /*for (int i = 0; i < MAX_HEIGHT; i++)
    {
      for (int j = 0; j < MAX_WIDTH; j++) 
    ...
    for (int i = 0; i < MAX_HEIGHT; i++) 
    {
      for (int j = 0; j < MAX_WIDTH/8; j++)
      {
    	  ...
    	  for(int k=0; k<8; k++){
			  outputs[i][j*8+k].x(31,0) = tmpframes(k*64+31,k*64+0);
			  outputs[i][j*8+k].y(31,0) = tmpframes(k*64+63,k*64+32);
    	  }
      }
    }
